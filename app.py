...
def upload_photo():
    ...
    if not file:
        return jsonify({"status": "error", "message": "photo file missing"}), 400
...
def api_ip_location():
    try:
        ...
        if not res.ok:
            return jsonify({"status": "error", "message": "ip lookup failed"}), 500

        data = res.json()
        if data.get("status") != "success":
            return jsonify({"status": "error", "message": "ip lookup not successful"}), 400
